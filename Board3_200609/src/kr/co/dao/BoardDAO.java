...
public class BoardDAO {
	...
	public pageTO page(int curPage) {
		...
		String sql = "select * from (select rownum rnum, num, title, writer, writeday, readcnt, repIndent from (select * from board order by repRoot desc, repStep asc))\r\n" + 
				"where rnum>=? and rnum <=?";
		...
		try {
			...
			while(rs.next()) { //최대 10개까지 들어가니 while문
				int num = rs.getInt("num");
				String title = rs.getString("title");
				String writer = rs.getString("writer");
				String writeday = rs.getString("writeday");
